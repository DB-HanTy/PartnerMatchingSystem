...
const doJoinTeam = async () => {
  ...
  if (res?.code === 0) {
    ...
    showToast({ type: 'fail', message: '加入失败' + (res.description ? `,${res.description}` : '') });
  }
}
...
const doQuitTeam = async (id: number) => {
  ...
  if (res?.code === 0) {
    ...
    showToast({ type: 'fail', message: '退出失败' + (res.description ? `,${res.description}` : '') });
  }
}
...
const doDeleteTeam = async (id: number) => {
  ...
  if (res?.code === 0) {
    showToast({ type: 'success', message: '解散成功' });
  } else {
    showToast({ type: 'fail', message: '解散失败' + (res.description ? `,${res.description}` : '') });
  }
}
